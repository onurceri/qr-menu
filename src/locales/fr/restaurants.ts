export default {
  title: 'Vos Restaurants',
  addNew: 'Ajouter un Restaurant',
  noRestaurants: 'Aucun Restaurant',
  noRestaurantsDesc: "Vous n'avez pas encore ajouté de restaurant. Créez votre premier restaurant en utilisant le formulaire ci-dessus pour commencer.",
  tip: '💡 Conseil: Après avoir créé un restaurant, vous pouvez ajouter des sections de menu et des plats pour présenter votre offre.',
  name: 'Nom du Restaurant',
  description: 'Description du Restaurant (optionnel)',
  creating: 'Création...',
  add: 'Ajouter Restaurant',
  deleting: 'Suppression...',
  menus: 'Menus',
  addMenu: 'Ajouter Menu',
  noMenus: 'Aucun menu ajouté',
  selectLanguage: 'Sélectionner la Langue',
  menuExists: 'Le menu dans cette langue existe déjà',
  menuInLanguage: 'Menu {{language}}',
  profile: 'Profil du Restaurant',
  uploadImage: 'Télécharger Image',
  imageRequirements: 'Maximum 5MB, format JPG ou PNG',
  address: 'Adresse',
  street: 'Rue',
  city: 'Ville',
  country: 'Pays',
  postalCode: 'Code Postal',
  defaultMenu: 'Menu par Défaut',
  imageUploadError: "Échec du téléchargement de l'image",
  loadError: 'Échec du chargement du restaurant',
  updateError: 'Échec de la mise à jour du restaurant',
  locationNotFound: 'Emplacement non trouvé',
  clickMapToSetLocation: 'Cliquez sur la carte pour définir manuellement l\'emplacement',
  location: 'Emplacement',
  hours: 'Heures d\'Ouverture',
  languages: 'Langues Disponibles',
  availableMenus: 'Menus Disponibles',
  fetchError: 'Échec de la récupération des détails du restaurant',
  notFound: 'Restaurant non trouvé',
  uploadingImage: 'Téléchargement de l\'image...',
  imageValidationError: 'Erreur de validation de l\'image: {{error}}',
  allowedFileTypes: 'Types de fichiers autorisés: {{types}}',
  isOpen: 'Ouvert',
  openTime: 'Heure d\'Ouverture',
  closeTime: 'Heure de Fermeture',
  closed: 'Fermé',
  noHours: 'Aucun horaire défini',
  noLocation: 'Emplacement non disponible',
  selectCountry: 'Sélectionner un pays',
  selectCity: 'Sélectionner une ville',
  descriptionHint: 'Une brève description de votre restaurant',
  mapInteractionHint: 'Cliquez et faites glisser pour déplacer la carte, utilisez les commandes de zoom pour zoomer/dézoomer'
}
